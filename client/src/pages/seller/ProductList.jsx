...
const ProductList = () => {
    ...
    const discardProduct = async (id) => {
        const confirm = window.confirm("Are you sure you want to discontinue this product?");
        if (!confirm) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.error(data.message || "Failed to discard product.");
            }
        } catch (error) {
            toast.error(error.message);
        }
                        ...
                        <tbody className="text-sm text-gray-500">
                            {products.map((product) => (
                                <tr key={product._id} className="border-t border-gray-500/20">
                                    ...
                                    <td className="px-4 py-3">
                                        <button
                                            onClick={() => discardProduct(product._id)}
