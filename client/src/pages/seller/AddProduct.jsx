...
const AddProduct = () => {
    ...
    const [files, setFiles] = useState([]);  // 0-2: images, 3: video
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                formData.append(i === 3 ? 'video' : 'images', files[i]);
            }
    ...
    return (
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                <div>
                    <p className="text-base font-medium">Product Media (3 images & 1 video)</p>
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`media${index}`}>
                                <input
                                    type="file"
                                    id={`media${index}`}
                                    accept={index === 3 ? 'video/*' : 'image/*'}
                                    hidden
                                    onChange={(e) => {
                ...
                <button className="px-8 py-2.5 bg-[#3f1f0a] text-white font-medium rounded cursor-pointer">ADD</button>
            </form>
        </div>
    );
};
